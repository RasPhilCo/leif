...
class AsserterBase {
    ...
    async run() {
        // 1.
        await utils_1.exec(`git -C ${this.workingDir} checkout master`); // branch from master
        try {
        ...
        if (this.assertion.if) {
            try {
                ...
                await utils_1.exec(`git -C ${this.workingDir} checkout master`);
                return;
            }
        }
        await this.uniqWork();
        ...
        if (stdout.toString().match(/nothing to commit, working tree clean/)) {
            ...
            await utils_1.exec(`git -C ${this.workingDir} commit -m "${this.commitDescription}" -m "Authored via leif asserted state"`);
